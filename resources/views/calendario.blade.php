...
<html lang=" str_replace('_', '-', app()->getLocale()) ">
    ...
    <body>
                ...
                <tr>
                  ...
                  <td>
                    <span class="btn-group btn-group-lg">
                        @if (mode !== 'day')
                          @if (mode === 'month')
                            <button class="js-cal-option btn btn-link" data-mode="year">{{ months[month] }}</button>
                          @endif
                          @if (mode === 'week')
                            <button class="btn btn-link disabled">{{ shortMonths[first.getMonth()] }} {{ last.getDate() }}</button>
                          @endif
                        @endif
      ...
      /*$.extend({
          quicktmpl: function (template) {return new Function("obj","p=[],print=function(){p.push.apply(p,arguments);};with(obj){p.push('"+template.replace(/[\r\t\n]/g," ").split("{{").join("\t").replace(/((^|\}\})[^\t]*)'/g,"$1\r").replace(/\t:(.*?)\}\}/g,"',$1,'").split("\t").join("');").split("}}").join("p.push('").split("\r").join("\\'")+"')}return p.join('');")}
      });*/
      ...
      (function ($) {
        ...
        t = $.quicktmpl($('#tmpl').get(0).innerHTML);
        ...
        function calendar($el, options) {
          ...
          function dayAddEvent(index, event) {
            ...
            $event = $('<div/>', {'class': 'event', text: event.title, title: event.title, 'data-index': index}),
                start = event.start,
                end = event.end || start,
      ...
      data = [],
        ...
        json_datos = <?php echo $myJson;?>;
        clases = json_datos['clases'];
        for (i = 0; i < clases.length; i++) {
          name = clases[i].name;
          color = clases[i].color;
          //--//
          time_start = clases[i].time_start;
          separador = ":";
          arregloDeSubCadenas = time_start.split(separador);
          h_start = arregloDeSubCadenas[0];
          m_start = arregloDeSubCadenas[1];
          //--//
          time_end = clases[i].time_end;
          separador = ":";
          arregloDeSubCadenas = time_end.split(separador);
          h_end = arregloDeSubCadenas[0];
          m_end = arregloDeSubCadenas[1];
          //--//
          day = clases[i].day;
          separador = "-";
          arregloDeSubCadenas = day.split(separador);
          ano = arregloDeSubCadenas[0];
          mes = arregloDeSubCadenas[1];
          dia = arregloDeSubCadenas[2];
          //--//
